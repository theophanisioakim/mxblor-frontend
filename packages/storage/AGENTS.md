...
- `src/storage.native.ts` — **native**: MMKV (`react-native-mmkv`) for ordinary values and Expo
  SecureStore for access, refresh, and schema-selection tokens; session store is cleared on cold
...
- Authentication secrets must use the native SecureStore mapping. Keep its physical key names in
  the supported alphanumeric/period/hyphen/underscore character set; never fall back to MMKV.
