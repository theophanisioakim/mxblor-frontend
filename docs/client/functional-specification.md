# Mxblor client functional specification
...
Mxblor is a shared building-management client that delivers the same screen implementation to:

- a browser application built with Next.js;
- a native application built with Expo and React Native.

The product combines a building portfolio, contacts, allocation/budget management, expense and
revenue catalogs, financial ledgers, reports, and a dashboard with the SBF administration
capabilities inherited from the sibling backend. It includes multi-schema authentication,
backend-controlled menus, endpoint-level permissions, localization, theming, responsive data
grids, and shared web/native screens.
...
- Public Mxblor landing page and authenticated portfolio dashboard.
- Building, unit, contact, related-person, communication, note, distribution, and yearly-budget
  workflows.
- Expense/revenue catalogs, building bank accounts, supplier expenses/payments, unit collections,
  balances, receipts, and building/share/financial reports.
...
- Backend business processing and database behavior, documented in `mxblor-backend`.
...
### FTR-013 — Building portfolio

Authorized users can manage buildings and their units, contacts, related people, communications,
notes, distribution tables, yearly budgets, and bank accounts. The dashboard and building hub
provide portfolio totals and direct navigation into these records.

### FTR-014 — Catalogs and ledgers

Authorized users can manage reusable expense/revenue catalogs and building-scoped expense, payment,
and collection ledgers. Create, edit, and delete controls are independently gated by exact backend
grants; generated or posted rows retain their documented edit restrictions.

### FTR-015 — Reports

Authorized users can request apartment/building shares, attendance, building financial, and
building-unit financial reports from the selected tenant context.
