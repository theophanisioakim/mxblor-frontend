...
- **Task runner:** **Turborepo** (`turbo.json`) — `build`, `dev`, `lint`, `format`, `typecheck`, `generate`
...
| `@workspace/i18n`              | storage                                               | i18next setup, type-safe `useTranslation`, locale JSON                                | app, apps                       |
...
| `@workspace/ui`                | web-ui, native-ui, api-client                         | cross-platform primitives (`Button`, `Text`, `View`, …)                               | app, apps                       |
| `@workspace/app`               | ui (+ api-client, i18n)                               | shared, cross-platform **screens**                                                    | apps                            |
| `@workspace/eslint-config`     | —                                                     | shared flat ESLint configs (`base`, `next-js`, `react-internal`)                      | all (dev)                       |
...
| Prettier check, no writes (CI/gate)         | `pnpm format:check`                                               |
...
> **Lint enforcement note.** `eslint-plugin-only-warn` downgrades every rule to a _warning_, so plain
> `pnpm lint` never fails. CI and the git hooks instead run **`pnpm lint:strict`**
> (`eslint --max-warnings 0`), which makes any warning — including the `web-ui`/`native-ui` import
> boundary (§2) — fail. Treat warnings on code you touch as must-fix.
...
- **Formatter:** Prettier (`.prettierrc`) — **no semicolons**, **double quotes**, 2-space indent,
  80-col, `trailingComma: es5`, `endOfLine: lf`, with `prettier-plugin-tailwindcss` (sorts classes;
  aware of `cn`/`cva`). Don't fight it — run `pnpm format`.
- **Linting:** ESLint flat config per workspace, extending `@workspace/eslint-config`
  (`base` / `next-js` / `react-internal`). `eslint-plugin-only-warn` is used, so lint surfaces as
  warnings — **leave zero warnings on code you touch** (treat them as must-fix, like the backend's
  "zero Sonar findings" rule).
...
├── package.json              ← root scripts (turbo) + engines + catalog dev deps + lint-staged
├── pnpm-workspace.yaml       ← workspaces, catalog (single-sourced versions), nodeLinker, overrides
├── turbo.json                ← task graph (build/dev/lint/format/typecheck/generate)
...
- **`pre-commit`** → `pnpm exec lint-staged`. Runs **`prettier --write`** on staged
  `*.{ts,tsx,js,jsx,json,md}` (formats only what you're committing — fast).
...
`eslint-plugin-only-warn` downgrades every rule to a warning, so `eslint` always exits 0. Both the
pre-push hook and CI run **`pnpm lint:strict`** (`eslint --max-warnings 0`) so warnings fail the
build — this is what actually enforces the **`web-ui`/`native-ui` import boundary** (§2), declared as
`@typescript-eslint/no-restricted-imports` in `@workspace/eslint-config` and relaxed only inside
`ui`/`web-ui`/`native-ui`.
